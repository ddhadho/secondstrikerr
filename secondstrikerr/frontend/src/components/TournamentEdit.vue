  ...
  <div class="q-gutter-md q-pa-md">
    ...
    <div class="col-12">
      ...
      <q-dialog v-model="showTeamsDialog">
        <q-card style="width: 300px; border-radius: 30px;">
          <q-card-section>
            <div class="text-h6">Teams per Group</div>
          </q-card-section>
          <q-card-section>
            <q-option-group
              ...
              @update:model-value="updateGroupOptions"
            />
          ...
          <q-card-actions align="right">
            <q-btn
              ...
              label="Save"
              color="secondary"
              @click="handleSaveTeamsConfig"
              :disable="!teamsPerGroup || !numberOfGroups"
            />
          </q-card-actions>
        </q-card>
      </q-dialog>
...
const tournamentId = route.params.competitionId;
...
async function handleSaveFixtures() {
  try {
    const response = await updateTournament({ fixtureType: fixtureType.value });
...
const teamsPerGroup = ref(null)
const numberOfGroups = ref(null)

const teamsPerGroupOptions = [
  { label: '3 Teams per Group', value: 3 },
  { label: '4 Teams per Group', value: 4 }
]

const availableGroups = computed(() => {
  if (teamsPerGroup.value === 3) {
    return [
      { label: '2 Groups', value: 2 },
      { label: '4 Groups', value: 4 },
      { label: '8 Groups', value: 8 },
      { label: '24 Groups', value: 24 }
    ]
  } else if (teamsPerGroup.value === 4) {
    return [
      { label: '2 Groups', value: 2 },
      { label: '4 Groups', value: 4 },
...
function updateGroupOptions() {
  numberOfGroups.value = null
}

async function handleSaveTeamsConfig() {
  try {
    const response = await updateTournament({
      teamsPerGroup: teamsPerGroup.value,
      numberOfGroups: numberOfGroups.value.value
    })
...
async function handleRules() {
  ...
  try {
    await updateTournament({ rules: rulesArray });
...
async function handleAwards() {
  ...
  try {
    const response = await updateTournament({ awards: awards.value });
    if (response.status === 200) {
      $q.notify({
        type: 'positive',
...
async function sendRequest() {
  if (selectedTeam.value) {
    try {
      const response = await api.post('/tournament/invite', {
...
async function startTournament() {
  if (!props.isAdmin) {
    $q.notify({
      type: 'warning',
      message: 'You are not authorized to start the tournament'
    });
    return;
  }
  if (props.competitionDetails.status !== 'draft') {
    $q.notify({
      type: 'warning',
      message: 'The tournament has already started or is completed'
    });
    return;
  }
  const expectedTeamsCount = props.competitionDetails.teamsPerGroup * props.competitionDetails.numberOfGroups;
  if (props.competitionDetails.members.length !== expectedTeamsCount) {
    $q.notify({
      type: 'warning',
      message: `You need exactly ${expectedTeamsCount} teams to start the league`
    });
    return;
  }
  ...
  try {
    const response = await api.patch(`/tournament/start/${tournamentId}`, {
      status: 'ongoing',
    });

    // Check response status before emitting
    if (response.status === 200) {
...
async function updateTournament(updatedFields) {
  try {
    const response = await api.patch(`/tournament/update/${tournamentId}`, updatedFields);
